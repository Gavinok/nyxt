...
Note that the function is executed *before* performing a return-action.")
     ...
     (auto-return-p nil
                    :type boolean
                    :documentation
                    "Automatically return and run the default `return-actions'
...
(defmethod (setf selection) (value (prompter prompter))
  ...
  (let ((source (selected-source prompter)))
    (when (auto-selection-action-p source)
      (if (< 0 (auto-return-selection-action-delay source))
          (run-thread "Prompter follow selection thread"
            (sleep (auto-return-selection-action-delay source))
...
(export-always 'toggle-auto-selection-action)
(defun toggle-auto-selection-action (prompter
                                     &optional (source (selected-source prompter)))
  "Toggle `auto-selection-action-p' in SOURCE."
  (setf (auto-selection-action-p source) (not (auto-selection-action-p source))))
